...
tournaments <- read.csv(file = here::here("data/EsportsEarnings.com/EE_tournaments.csv"))

games <- read.csv(file = here::here("data/EsportsEarnings.com/EE_games_earnings.csv")) %>%
...
inputPanel(
  selectInput("gameName", label = "Game:",
              choices = o_prizes$GameName, selected = o_prizes$GameName[1]),
  
  sliderInput("dateRange", label = "Date range of observations:",
...
inputPanel(
  selectInput("gameId", label = "Game ID to display:",
              choices = o_tourn$GameName, selected = o_tourn$GameName[1]),
