...
From these visualizations, we can identify 5 particularly interesting games:
...
- Second, some games have regular peaks in prize pools that do not match the peaks of number of tournaments. Those are usually major international events, such as `League of Legends: World Championship`. Here are a few (same game as selected above):
```{r}
...
```
  As we can see for games like Dota 2, Starcraft II or League of Legends, the highest paying tournaments are the World Championships (`The International` for Dota 2, `World Championship` for LoL, `WCS` for StarCraft II). The story is a bit more complicated for other genre games such as Counter Strike.
- Finally, 

## Prediction

Based on these observations, we could try to build a model to predict the evolution of e-sports' earnings in the future. Indeed, excepting 2020, which saw the COVID-19 pandemic put a halt to most physical events, we can see that prize pools increase steadily for some games.