...
```{r child = 'esports_earnings.Rmd'}
```

```{r child = 'test_twitch.Rmd'}
```

```{r child = 'test_NewZoo.Rmd'}
```
...
Finally, although our data from investment types is not sufficient to give meaningful insights, it does show that the amount of money involved in this market are not trivial, and that it is growing.
